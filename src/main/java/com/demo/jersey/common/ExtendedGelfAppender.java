...
public class ExtendedGelfAppender extends AppenderSkeleton implements
		GelfMessageProvider {
	...
	public String getFacility() {
		if (facility == null || facility.equals("gelf-java")) {
			facility = getLocalHostIP();
		}
		return facility;
	}

	public void setFacility(String facility) {
		ExtendedGelfAppender.facility = facility;
	}
		...
		try {
			...
			while (allNetInterfaces.hasMoreElements()) {
				...
				while (addresses.hasMoreElements()) {
					hostIP = (InetAddress) addresses.nextElement();
					if (hostIP != null && hostIP instanceof Inet4Address
							&& !hostIP.isLoopbackAddress()) {
						return hostIP.getHostAddress();
					}
				}
			}
		} catch (SocketException e) {
			errorHandler.error("Unknown local hostip", e,
					ErrorCode.GENERIC_FAILURE);
		}
		return "Unknown local hostip";
	}
	...
	public void setOriginHost(String originHost) {
		ExtendedGelfAppender.originHost = originHost;
	}
	...
	public void activateOptions() {
		if (graylogHost == null && amqpURI == null) {
			errorHandler.error("Graylog2 hostname and amqp uri are empty!",
					null, ErrorCode.WRITE_FAILURE);
		} else if (graylogHost != null && amqpURI != null) {
			errorHandler.error(
					"Graylog2 hostname and amqp uri are both informed!", null,
			...
			try {
				...
				} else if (graylogHost != null
						&& graylogHost.startsWith("udp:")) {
					String udpGraylogHost = graylogHost.substring(4);
					gelfSender = getGelfUDPSender(udpGraylogHost, graylogPort);
				} else if (amqpURI != null) {
					gelfSender = getGelfAMQPSender(amqpURI, amqpExchangeName,
							amqpRoutingKey, amqpMaxRetries);
				} else {
					gelfSender = getGelfUDPSender(graylogHost, graylogPort);
				}
			} catch (UnknownHostException e) {
				errorHandler.error("Unknown Graylog2 hostname:"
						+ getGraylogHost(), e, ErrorCode.WRITE_FAILURE);
			} catch (SocketException e) {
				...
				errorHandler.error("AMQP uri exception", e,
						ErrorCode.WRITE_FAILURE);
			} catch (NoSuchAlgorithmException e) {
				errorHandler.error("AMQP algorithm exception", e,
						ErrorCode.WRITE_FAILURE);
			} catch (KeyManagementException e) {
				errorHandler.error("AMQP key exception", e,
						ErrorCode.WRITE_FAILURE);
			}
		}
	}

	protected GelfUDPSender getGelfUDPSender(String udpGraylogHost,
			int graylogPort) throws IOException {
		return new GelfUDPSender(udpGraylogHost, graylogPort);
	}

	protected GelfTCPSender getGelfTCPSender(String tcpGraylogHost,
			int graylogPort) throws IOException {
		return new GelfTCPSender(tcpGraylogHost, graylogPort);
	}

	protected GelfAMQPSender getGelfAMQPSender(String amqpURI,
			String amqpExchangeName, String amqpRoutingKey, int amqpMaxRetries)
			throws IOException, URISyntaxException, NoSuchAlgorithmException,
			KeyManagementException {
		return new GelfAMQPSender(amqpURI, amqpExchangeName, amqpRoutingKey,
				amqpMaxRetries);
	}

	@Override
	protected void append(LoggingEvent event) {
		GelfMessage gelfMessage = GelfMessageFactory.makeMessage(layout, event,
				this);
